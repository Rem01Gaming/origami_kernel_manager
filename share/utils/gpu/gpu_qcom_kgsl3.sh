...
gpu_qcom_kgsl3_set_max_freq() {
	if [[ $1 == "-exec" ]]; then
		...
		command2db gpu.qcom.kgsl3.max_freq "gpu_qcom_kgsl3_set_max_freq -exec $freq" FALSE
	fi
...
gpu_qcom_kgsl3_set_gov() {
	if [[ $1 == "-exec" ]]; then
		...
		command2db gpu.qcom.kgsl3.governor "gpu_qcom_kgsl3_set_gov -exec $selected_gov" FALSE
	fi
