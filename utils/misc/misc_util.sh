...
misc_menu() {
	while true; do
		...
		echo -e "\e[30;48;2;254;228;208;38;2;0;0;0m Origami Kernel Manager v1.0.1$(yes " " | sed $(($LINE - 30))'q' | tr -d '\n')\033[0m"
		echo -e "\e[38;2;254;228;208m"
		echo -e "    _________      [] Storage: ${storage_manufacturer} ${storage_codename}" | cut -c 1-${LINE}
		echo -e "   /        /\\     [!] Storage Status: $storage_status"
		echo -e "  /        /  \\    [] Thermal Governor: $(cat /sys/class/thermal/thermal_zone0/policy)"
		echo -e " /        /    \\   [] SELINUX: $(getenforce)"
		echo -e "/________/      \\  $(echo "$misc_menu_info" | awk -F '//' '{print $1}')"
		echo -e "\\        \\      /  $(echo "$misc_menu_info" | awk -F '//' '{print $2}')"
		echo -e " \\        \\    /   $(echo "$misc_menu_info" | awk -F '//' '{print $3}')"
		echo -e "  \\        \\  /    $(echo "$misc_menu_info" | awk -F '//' '{print $4}')"
		echo -e "   \\________\\/     $(echo "$misc_menu_info" | awk -F '//' '{print $5}')"
		echo -e "\n//////////////"
